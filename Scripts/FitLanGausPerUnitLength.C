...
void FitLanGausPerUnitLength(){
   ...
   * setup
   */
  const int n_dedx_bins = 100;
  const double max_dedx = 50;
  const int n_resrg_bins = 15;

  double landau_widths[n_resrg_bins];
  double chisq_ndfs[n_resrg_bins];
  double resrgs[n_resrg_bins];

  TFile *file = new TFile("/uboone/data/users/kduffy/PIDproducer_UBXSec_BNBCosmic_v1/particleIdMeta_merged_20180516.root", "read");
  ...
  tree->Draw("track_dEdx_perhit:track_resrange_perhit >> h_protons", 
      "true_PDG == 2212 && true_start_x > 10 && true_start_x < 246 && true_end_x > 10 && true_end_x < 246 && true_start_y > -106.5 && true_start_y < 106.5 && true_end_y > -106.5 && true_end_y < 106.5 && true_start_z > 10 && true_start_z < 1030 && true_end_z > 10 && true_end_z < 1030");
  ...
  gr_landau_widths->Draw("AP");
  gr_chisq_ndf->Draw("sameP");
