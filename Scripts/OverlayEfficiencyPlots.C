...
std::vector<double> marker_sizes = {0.4,0.4,0.4};
  ...
  for (size_t i_plot=0; i_plot<plotnames.size(); i_plot++){
    ...
    for (size_t i_file=0; i_file < inputfiles.size(); i_file++){
      ...
      for (size_t i_p=0; i_p<particles.size(); i_p++){
        ...
        heff->SetLineWidth(1);
        heff->SetMarkerColor(kGreen+1);
        heff->SetMarkerStyle(markerstyle);
        heff->SetMarkerSize(markersize);

        hpur->SetFillStyle(0);
        hpur->SetLineColor(kAzure+1);
        hpur->SetLineWidth(1);
        hpur->SetMarkerColor(kAzure+1);
        ...
        if (i_p==1 && i_file==0){
          ...
          if (legendy<0.6) l = new TLegend(0.12,0.3,1.0,0.88);
          else l = new TLegend(0.12,0.3,1.0,0.88);
          //l->SetTextFont(132);
